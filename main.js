      ...
      listHtml += `
        <div class="skill-row" data-name="${skill.name}" style="cursor:help;">
          <div class="skill-info">
            ...
            <span class="skill-pct">${skill.rating}%</span>
          </div>
...
function renderSidebarFilters() {
  ...
  if (activeArchiveTab === 'designs') {
    ...
    filters = Array.from(softwares);
  } else if (activeArchiveTab === 'videos') {
    title = 'Categories';
    const categories = new Set();
    PROFILE_DATA.videos.forEach(v => categories.add(v.category));
    filters = Array.from(categories);
  }
...
function filterArchiveData() {
  ...
  if (activeArchiveTab === 'designs') {
    ...
    const filtered = PROFILE_DATA.videos.filter(vid => {
      const matchesSearch = vid.title.toLowerCase().includes(query) || vid.description.toLowerCase().includes(query) || vid.tags.some(t => t.toLowerCase().includes(query));
      const matchesFilter = selectedFilter === 'all' || vid.category === selectedFilter;
      return matchesSearch && matchesFilter;
    });
    renderArchiveVideos(filtered);
  }
}
...
// 6. Videos Hub Render with Fallback Radar scanner
function renderArchiveVideos(items) {
  const grid = document.getElementById('explorer-videos-grid');
  grid.innerHTML = '';

  if (items.length === 0) {
    grid.innerHTML = `<div style="grid-column:1/-1; text-align:center; padding:40px 0; color:var(--text-secondary);">No video records match filters.</div>`;
    return;
  }
  ...
  items.forEach(vid => {
    ...
    grid.innerHTML += `
      <div class="video-card">
        <div class="video-thumbnail-box" onclick="openVideoPlayer('${vid.filePath}', '${vid.title}', '${vid.description}')">
          <div class="video-duration-badge">${vid.duration}</div>
        </div>
        <div class="video-info-box">
          <span class="featured-cat-tag">${vid.category} // ${vid.date}</span>
          <h4>${vid.title}</h4>
          <p>${vid.description}</p>
          
          <div class="pres-meta-footer" style="border:none; padding-top:0; margin-top:auto;">
            <div class="meta-row" style="font-size:0.75rem; color:var(--text-secondary); margin-bottom:12px;">
              <span>Associated Project:</span>
              <span onclick="triggerCrossLink('${vid.relatedContent.link}')" class="crosslink-tag">${vid.relatedContent.text}</span>
            </div>
            <div class="proj-modal-tech" style="margin-bottom:18px;">${tagsHtml}</div>
            
            <button onclick="openVideoPlayer('${vid.filePath}', '${vid.title}', '${vid.description}')" class="btn-primary" style="padding:8px 14px; font-size:0.75rem; border-radius:6px; width:100%;">
              <span class="material-symbols-outlined" style="font-size:14px;">play_circle</span> Initialize Playback
            </button>
          </div>
        </div>
      </div>
...
function openVideoPlayer(filePath, title, description) {
  const modal = document.getElementById('video-preview-modal');
  const video = document.getElementById('archive-html5-video');
  const source = document.getElementById('video-source-node');
  const titleHeader = document.getElementById('video-modal-title');
  const descText = document.getElementById('video-modal-desc');
  ...
  if (playPromise !== undefined) {
    playPromise.catch(error => {
      // Auto-catch autoplay policies blocks
      console.log("Autoplay block (standard browser behavior):", error);
    });
  }
...
function handleVideoLoadError(videoNode) {
  // If local .mp4 is missing, trigger the premium telemetry scanning radar fallback
  videoNode.style.display = 'none';
  document.getElementById('video-fallback-card').style.display = 'flex';
}
...
function initUniversalSearch() {
  ...
  searchInput.addEventListener('input', () => {
    ...
    const results = {
      ...
      videos: PROFILE_DATA.videos.filter(v => v.title.toLowerCase().includes(query) || v.description.toLowerCase().includes(query) || v.tags.some(t => t.toLowerCase().includes(query)))
    };
...
function renderSearchResults(results, query) {
  ...
  if (results.videos.length > 0) {
    totalCount += results.videos.length;
    let group = `<div class="search-result-group"><h5>Videos (${results.videos.length})</h5>`;
    results.videos.forEach(v => {
      group += `<div class="search-result-item" onclick="selectSearchResult('video', '${v.filePath}', '${v.title}', '${v.description}')">
        <span>${v.title}</span>
        <span class="search-result-type-tag">Simulation Video</span>
      </div>`;
    });
    group += `</div>`;
    container.innerHTML += group;
...
function selectSearchResult(type, param1, param2, param3) {
  ...
  if (type === 'project') {
    ...
    openVideoPlayer(param1, param2, param3);
  } else {
...
function initModals() {
  ...
  modals.forEach(modalId => {
    ...
    const closeBtn = modal.querySelector('.modal-close-btn') || modal.querySelector('.lightbox-close-btn') || modal.querySelector('.drawer-close-btn') || document.getElementById('pres-modal-close') || document.getElementById('video-modal-close');
    if (closeBtn) {
...
function closeSpecificModal(modal) {
  ...
  // Custom video cutout action
  if (modal.id === 'video-preview-modal') {
    const video = document.getElementById('archive-html5-video');
    video.pause();
    video.currentTime = 0;
  }
